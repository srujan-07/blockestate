...
export default function LandSearch() {
  ...
  // Hyperledger Fabric Configuration (matches running backend on port 4000)
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          throw new Error(msg || "Land record not found. Please verify district, mandal, village, and survey/property ID.");
        }
      ...
      if (searchType === "survey") {
        ...
        if (blockchainData.ipfsCID) {
          try {
            ...
            console.warn("IPFS fetch failed, showing on-chain data only", ipfsErr);
            setLandData({ ...onChainData, ipfsCID: blockchainData.ipfsCID });
          }
        } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-100 via-blue-100 to-purple-100 p-10 flex justify-center">
      <div className="max-w-5xl w-full mt-10 backdrop-blur-xl bg-white/40 shadow-2xl border border-white/30 rounded-3xl p-10">
            ...
            <div className="grid grid-cols-3 gap-6 mb-6">
              ...
              <div>
                ...
                >
                  ...
                  <option value="Fathima Nagar">Fathima Nagar</option>
        ...
        {/* ERROR MESSAGE */}
        {error && (
          <div className="mt-6 p-4 rounded-xl bg-red-100 border border-red-300 text-red-700 font-semibold">
            {error}
          </div>
        )}

...
export default function LandSearch() {
  ...
  const HYPERLEDGER_API = "http://localhost:4000"; // Backend API endpoint
    ...
    try {
      ...
      if (queryType === "survey") {
        ...
        body = {
          ...
          surveyNo: queryKey.trim()
        };
      } else {
        endpoint = `${HYPERLEDGER_API}/land/query-by-id`;
        body = {
          propertyId: queryKey.trim()
        };
      }

      console.log(`Fetching from ${endpoint}`, body);
      ...
      if (!response.ok) {
        if (response.status === 404) {
          throw new Error(data.error || "Land record not found");
        }
  ...
  const handleFetch = async () => {
    ...
    try {
      ...
      // Don't show fallback data on error - only actual matches should show data
    } finally {
      setLoading(false);
    }
  };
